...
Deno.test("Test each component of operational principle for Arc Tracking", async (t) => {
  await t.step("should allow users to create and manage arcs", async () => {
    ...
    // Now newest first due to _id descending
    assertEquals(userArcs.arcs, [arc2Id, arc1Id]); 
  ...
  await t.step("should order arcs by streak count and name", async () => {
    ...
    const { arc: arcA } = await arcTracker.createArc({ name: "Arc A", members: [userAlice] }); // streak 0
    const { arc: arcB } = await arcTracker.createArc({ name: "Arc B", members: [userAlice] }); // streak 0
    const { arc: arcC } = await arcTracker.createArc({ name: "Arc C", members: [userAlice] }); // streak 0

    // Manually set streaks for testing order
    await db.collection("ArcTracking.arcs").updateOne({ _id: arcA }, { $set: { streak: 5, name: "Arc A" } });
    await db.collection("ArcTracking.arcs").updateOne({ _id: arcB }, { $set: { streak: 2, name: "Arc B" } });
    await db.collection("ArcTracking.arcs").updateOne({ _id: arcC }, { $set: { streak: 5, name: "Arc C" } });

    const userArcs = await arcTracker.getArcs({ user: userAlice });

    // Sorted by streak DESC, then _id DESC (newest first)
    // So between arcA and arcC (both streak 5), arcC is newer → comes first
    assertEquals(userArcs.arcs, [arcC, arcA, arcB]);
