...
content_id: 77a05bee303f6dd4a55713f3ebc86c6218c544f979da497e8ab18c0bfeaecfb8
---
...
a set of FriendRequests with
    a sender FromUser
    a receiver User
    a status String (e.g., "pending", "accepted", "rejected")
```
...
```
acceptRequest (friendRequest: FriendRequest): (user: User, friend: User)
    requires the friendRequest status is "pending"
    requires the authenticated user is the receiver of the friendRequest
    effect updates the friendRequest status to "accepted", adds the sender to the receiver's friends, and adds the receiver to the sender's friends, and returns the authenticated user and the sender
